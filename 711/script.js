...
calculateButton.addEventListener('click', () => {
  fetch('https://data.cityofnewyork.us/resource/qgc5-ecnb.json')
    ...
    .then(data => {
      console.log(data);
      const sugarData = data.filter(item => item.restaurant && item.restaurant.toLowerCase().includes("7 eleven") && item.sugar !== undefined);

      console.log('Items with defined restaurant attribute:', data.filter(item => item.restaurant));
      if (sugarData.length === 0) {
        console.log('No data found for the Sugar category or 7 Eleven restaurant.');
        return;
      }
